...
public class Activator implements BundleActivator {
    ...
    private static class ServiceCache {
        ...
        <T> Collection<ServiceReference<T>> getReferences(Class<T> clazz) {
        	Collection<ServiceReference<T>> hit = (Collection<ServiceReference<T>>) cache.get(clazz);
        	if (hit != null) {
        		return hit;
        	}
        	else {
        		synchronized (trackers) {
        			hit = (Collection<ServiceReference<T>>) cache.get(clazz);
        			if (hit == null) {
        				hit = (Collection<ServiceReference<T>>) new CachingServiceTracker<>(context, clazz).getReferences();
        			}
        		}
        		return hit;
        	}
